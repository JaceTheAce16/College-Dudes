...
const ALLOWED_OWNERS = [
  'jacedjessup@gmail.com',
  (import.meta as any).env?.VITE_OWNER_EMAIL
].filter(Boolean).map(e => e.toLowerCase());

function isAuthorizedOwner(email?: string | null): boolean {
  if (!email) return false;
  return ALLOWED_OWNERS.includes(email.toLowerCase());
}
...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (isAuthorizedOwner(currentUser.email)) {
          fetchLeadsDirectly(currentUser);
        } else {
          setLeads([]);
        }
      } else {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        if (isAuthorizedOwner(result.user.email)) {
          fetchLeadsDirectly(result.user);
        } else {
          setLeads([]);
        }
      }
  ...
  const fetchLeadsDirectly = async (currentUser: FirebaseUser) => {
    if (!currentUser || !isAuthorizedOwner(currentUser.email)) {
      setLeads([]);
      return;
    }
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        {isOpen && (
          <div className="mt-8 bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
            {user && !isAuthorizedOwner(user.email) ? (
              <div className="py-8 text-center space-y-4 max-w-md mx-auto">
                ...
                <div>
                  <h3 className="text-lg font-black text-brand-navy uppercase tracking-tight">Access Restricted</h3>
                  <p className="text-sm text-slate-600 mt-2 leading-relaxed">
                    The logged in account <strong className="text-red-500">{user.email}</strong> is not authorized to view leads or synchronize events.
                  </p>
                </div>
                <div className="pt-2">
                  <button
                    onClick={handleLogout}
                    className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs px-4 py-2.5 rounded-xl transition cursor-pointer mx-auto"
                  >
                    <LogOut size={14} /> Sign Out / Try Another Account
                  </button>
                </div>
              </div>
            ) : (
